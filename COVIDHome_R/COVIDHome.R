...
#==============================================================================#
url <-
  "https://docs.google.com/spreadsheets/d/1EoGQqiZIAtq-4_-hEi0JEgbgf9-_tqXbsQnCZBQ8x1w/edit?usp=sharing"
...
a <-
  str_replace(a, "Did you test positive for COVID-19\\?", "Positive")
a <- str_replace(a, "What country are you in\\?", "Country")
a <-
  str_replace(a, "What state are you in\\? (Choose NA if not in U.S.)", "State")
a <-
  str_replace(a,
              "What state/region/province are you in? (Write 'NA' if in U.S.)",
              "State")
a <-
  str_replace(a, "What county (or equivalent) are you in\\?", "County")
a <-
  str_replace(a, "What city (or equivalent) are you in\\?", "City")

# Make the data into a dataframe
data <-
  as.data.frame(read.table(text = a, sep = sep),
                skip = 2,
                headers = T)
data <- data[-1]
data$V1 <- NULL
...
saveWidget(
  out4,
  "/Users/patrick/Dropbox (University of Oregon)/Github/phorve.github.io/COVIDHome_R/html/p4.html",
  selfcontained = T,
  libdir = "lib"
)
...
rollingcount <-
  read.csv(
    "/Users/patrick/Dropbox (University of Oregon)/Github/phorve.github.io/COVIDHome_R/rollingcount.csv"
  )
...
test <- test1 == test2
if (test == FALSE) {
  hold.table <- data.frame("Date" = format(Sys.time(), "%m/%d/%y"),
                           "Tests" = add)
  rollingcount <- rbind(hold.table, rollingcount)
  rollingcount = rollingcount[order(as.Date(rollingcount$Date, format =
                                              "%m/%d/%y")), ]
  write_csv(
    rollingcount,
    "/Users/patrick/Dropbox (University of Oregon)/Github/phorve.github.io/COVIDHome_R/rollingcount.csv"
...
r <-
  ggplot(rollingcount, aes(as.Date(Date, format = "%m/%d/%y"), Tests)) +
  geom_bar(stat = "identity") +
  theme_classic() +
  ylab("Total Tests Reported") +
  xlab("Date") +
...
saveWidget(
  out2,
  "/Users/patrick/Dropbox (University of Oregon)/Github/phorve.github.io/COVIDHome_R/html/p3.html",
  selfcontained = T,
  libdir = "lib"
)
...
locations.combined$Submissions[is.na(locations.combined$Submissions)] <-
  0
...
state2 <- ggplot() +
  ...
  theme_void() +
  geom_text(data = locations.combined,
            aes(long, lat, label = Submissions),
            color = "Blue") +
  geom_text(aes((locations.combined$long[2] - 1),
                (locations.combined$lat[2] - 1),
                label = "Alaska"
  )) +
  geom_text(aes((locations.combined$long[11] - 1),
                (locations.combined$lat[11] - 1),
                label = "Hawaii"
  )) +
...
saveWidget(
  out6,
  "/Users/patrick/Dropbox (University of Oregon)/Github/phorve.github.io/COVIDHome_R/html/p6.html",
  selfcontained = T,
  libdir = "lib"
)

#==============================================================================#
# Summarize the testing data based on the test that was used
#==============================================================================#
tests <- data %>%
  select(Date, Test)
...
tests_final <-
  rbind(abbott, Quidel, iHealth, Ellume, On, CVX, unk, intelli, ff, cvs)
...
saveWidget(
  out1,
  "/Users/patrick/Dropbox (University of Oregon)/Github/phorve.github.io/COVIDHome_R/html/p2.html",
  selfcontained = T,
  libdir = "lib"
)

#==============================================================================#
# Summarize our data (COVID positives) from each state
#==============================================================================#
summary_state_positive <- data %>%
  filter(Positive == 1)
summary_state_positive <-
  ddply(
    summary_state_positive,
    .(State, County, City, Date),
    .drop = TRUE,
    summarise,
    Cases = length(Positive)
  )
...
saveWidget(
  out,
  "/Users/patrick/Dropbox (University of Oregon)/Github/phorve.github.io/COVIDHome_R/html/p1.html",
  selfcontained = T,
  libdir = "lib"
)
...
state1 <- ggplot() +
  ...
  scale_fill_continuous(
    name = "Number of Positive Rapid Tests",
    low = "white",
    high = "#8B0000",
    limits = c(0, 15)
  )
...
saveWidget(
  out5,
  "/Users/patrick/Dropbox (University of Oregon)/Github/phorve.github.io/COVIDHome_R/html/p5.html",
  selfcontained = T,
  libdir = "lib"
)
...
# Paths for our files
output1 <-
  paste("/Volumes/GoogleDrive/My Drive/HomeCOVID/rawdata/rawdata_",
        date,
        ".csv",
        sep = "")
output2 <-
  paste(
    "/Volumes/GoogleDrive/My Drive/HomeCOVID/summarydata/summarydata_",
    date,
    ".csv",
    sep = ""
  )
output3 <-
  paste("/Volumes/GoogleDrive/My Drive/HomeCOVID/testdata/testdata_",
        date,
        ".csv",
        sep = "")

# Save the files for public access
